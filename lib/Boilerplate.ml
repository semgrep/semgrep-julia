...
let map_tok_abst_pat_3d340f6_type (env : env) (tok : CST.tok_abst_pat_3d340f6_type) =
...
let map_tok_prim_pat_3d340f6_type (env : env) (tok : CST.tok_prim_pat_3d340f6_type) =
  (* tok_prim_pat_3d340f6_type *) token env tok
...
let map_pat_aa33ccb (env : env) (tok : CST.pat_aa33ccb) =
  (* pattern \$[A-Z][a-zA-Z0-9]* *) token env tok
...
let map_identifier (env : env) (x : CST.identifier) =
  ...
  | `Pat_aa33ccb x -> R.Case ("Pat_aa33ccb",
      map_pat_aa33ccb env x
    )
  )
...
and map_anon_choice_assign_b59cd00 (env : env) (x : CST.anon_choice_assign_b59cd00) =
  (match x with
  | `Assign x -> R.Case ("Assign",
      map_assignment env x
    )
  | `Bare_tuple x -> R.Case ("Bare_tuple",
      map_bare_tuple env x
    )
  | `Id x -> R.Case ("Id",
      map_identifier env x
    )
  ...
  | `Short_func_defi x -> R.Case ("Short_func_defi",
      map_short_function_definition env x
    )
  )

and map_anon_choice_exp_0ff8d07 (env : env) (x : CST.anon_choice_exp_0ff8d07) =
  (match x with
  | `Exp x -> R.Case ("Exp",
...
and map_anon_choice_exp_772c79a (env : env) (x : CST.anon_choice_exp_772c79a) =
  (match x with
  | `Exp x -> R.Case ("Exp",
      map_expression env x
    )
...
and map_anon_choice_exp_91c2553 (env : env) (x : CST.anon_choice_exp_91c2553) =
  ...
  | `Named_field x -> R.Case ("Named_field",
      map_named_field env x
    )
  )
...
and map_anon_choice_exp_rep_COMMA_choice_exp_7e6cb67 (env : env) ((v1, v2) : CST.anon_choice_exp_rep_COMMA_choice_exp_7e6cb67) =
...
and map_anon_choice_id_6314bc3 (env : env) (x : CST.anon_choice_id_6314bc3) =
  ...
  | `Slurp_param x -> R.Case ("Slurp_param",
      map_slurp_parameter env x
    )
...
and map_anon_choice_id_687d935 (env : env) (x : CST.anon_choice_id_687d935) =
  ...
  | `Opt_param x -> R.Case ("Opt_param",
      map_optional_parameter env x
    )
  | `Typed_param x -> R.Case ("Typed_param",
      map_typed_parameter env x
    )
  ...
  | `Closed_macr_exp x -> R.Case ("Closed_macr_exp",
      map_closed_macrocall_expression env x
    )
  )
...
and map_anon_choice_id_6965274 (env : env) (x : CST.anon_choice_id_6965274) =
  ...
  | `LPAR_choice_id_RPAR (v1, v2, v3) -> R.Case ("LPAR_choice_id_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = map_anon_choice_id_6314bc3 env v2 in
...
and map_argument_list (env : env) ((v1, v2, v3, v4, v5) : CST.argument_list) =
  ...
  let v2 =
    ...
    | Some x -> R.Option (Some (
        (match x with
        | `Choice_exp_rep_COMMA_choice_exp_opt_COMMA_opt_exp_comp_clause (v1, v2, v3) -> R.Case ("Choice_exp_rep_COMMA_choice_exp_opt_COMMA_opt_exp_comp_clause",
            let v1 = map_anon_choice_exp_91c2553 env v1 in
            let v2 =
              R.List (List.map (fun (v1, v2) ->
                let v1 = (* "," *) token env v1 in
  ...
  let v3 =
    ...
    | Some (v1, v2) -> R.Option (Some (
        ...
        let v2 =
          (match v2 with
          | Some x -> R.Option (Some (
              map_anon_choice_exp_rep_COMMA_choice_exp_7e6cb67 env x
            ))
          | None -> R.Option None)
        in
        R.Tuple [v1; v2]
...
and map_assignment (env : env) ((v1, v2, v3) : CST.assignment) =
  let v1 =
    ...
    | `Para_type_exp x -> R.Case ("Para_type_exp",
        map_parametrized_type_expression env x
      )
    | `Interp_exp x -> R.Case ("Interp_exp",
        map_interpolation_expression env x
      )
...
and map_bare_tuple (env : env) ((v1, v2) : CST.bare_tuple) =
  let v1 = map_expression env v1 in
  let v2 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
...
and map_block (env : env) ((v1, v2, v3) : CST.block) =
  let v1 = map_anon_choice_exp_772c79a env v1 in
  let v2 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = map_terminator env v1 in
      let v2 = map_anon_choice_exp_772c79a env v2 in
      R.Tuple [v1; v2]
    ) v2)
  in
  let v3 =
...
and map_closed_macrocall_expression (env : env) ((v1, v2, v3) : CST.closed_macrocall_expression) =
  let v1 =
    (match v1 with
    | Some (v1, v2) -> R.Option (Some (
        let v1 = map_primary_expression env v1 in
...
and map_definition (env : env) (x : CST.definition) =
  ...
  | `Abst_defi (v1, v2, v3, v4, v5) -> R.Case ("Abst_defi",
  ...
  | `Prim_defi (v1, v2, v3, v4, v5, v6) -> R.Case ("Prim_defi",
      let v1 = map_tok_prim_pat_3d340f6_type env v1 in
      let v2 = map_anon_choice_id_00cc266 env v2 in
      let v3 =
        (match v3 with
        | Some (v1, v2) -> R.Option (Some (
      ...
      let v4 =
        ...
        | None -> R.Option None)
      in
      let v5 = map_integer_literal env v5 in
  ...
  | `Macro_defi (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Macro_defi",
...
and map_function_definition (env : env) ((v1, v2, v3) : CST.function_definition) =
  ...
  let v2 =
    (match v2 with
    | `Choice_func_sign_opt_choice_LF_opt_blk (v1, v2, v3) -> R.Case ("Choice_func_sign_opt_choice_LF_opt_blk",
        let v1 =
          (match v1 with
          | `Func_sign x -> R.Case ("Func_sign",
              map_function_signature env x
            )
          | `Param_list_opt_COLONCOLON_prim_exp_opt_where_clause (v1, v2, v3) -> R.Case ("Param_list_opt_COLONCOLON_prim_exp_opt_where_clause",
    ...
    | `Choice_id x -> R.Case ("Choice_id",
        map_anon_choice_id_267a5f7 env x
      )
    )
  in
...
and map_function_signature (env : env) ((v1, v2, v3, v4, v5, v6) : CST.function_signature) =
  let v1 =
    (match v1 with
    | `Id x -> R.Case ("Id",
        map_identifier env x
    ...
    | `LPAR_typed_param_RPAR (v1, v2, v3) -> R.Case ("LPAR_typed_param_RPAR",
        let v1 = (* "(" *) token env v1 in
        let v2 = map_typed_parameter env v2 in
        let v3 = (* ")" *) token env v3 in
        R.Tuple [v1; v2; v3]
      )
    | `Interp_exp x -> R.Case ("Interp_exp",
...
and map_global_statement (env : env) ((v1, v2) : CST.global_statement) =
  ...
  let v2 = map_anon_choice_assign_b59cd00 env v2 in
  R.Tuple [v1; v2]
...
and map_importable (env : env) (x : CST.importable) =
  ...
  | `Rela_qual (v1, v2) -> R.Case ("Rela_qual",
...
and map_interpolation_expression (env : env) (x : CST.interpolation_expression) =
  (match x with
  | `DOLLAR_choice_num (v1, v2) -> R.Case ("DOLLAR_choice_num",
      let v1 = (* "$" *) token env v1 in
      let v2 =
        (match v2 with
        | `Num x -> R.Case ("Num",
...
and map_keyword_parameters (env : env) ((v1, v2, v3, v4) : CST.keyword_parameters) =
  let v1 = (* ";" *) token env v1 in
...
and map_macro_argument_list (env : env) (xs : CST.macro_argument_list) =
  R.List (List.map (map_anon_choice_exp_772c79a env) xs)
...
and map_matrix_row (env : env) (xs : CST.matrix_row) =
  R.List (List.map (map_anon_choice_exp_91c2553 env) xs)
...
and map_operation (env : env) (x : CST.operation) =
  ...
  | `Juxt_exp (v1, v2) -> R.Case ("Juxt_exp",
      let v1 =
        (match v1 with
        | `Int_lit x -> R.Case ("Int_lit",
            map_integer_literal env x
  ...
  | `Comp_assign_exp (v1, v2, v3) -> R.Case ("Comp_assign_exp",
      let v1 = map_primary_expression env v1 in
      let v2 =
        (match v2 with
...
and map_parametrized_type_expression (env : env) ((v1, v2, v3) : CST.parametrized_type_expression) =
  let v1 = map_primary_expression env v1 in
...
and map_parenthesized_expression (env : env) ((v1, v2, v3, v4, v5, v6) : CST.parenthesized_expression) =
  let v1 = (* "(" *) token env v1 in
  let v2 = map_anon_choice_exp_0ff8d07 env v2 in
  let v3 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* ";" *) token env v1 in
      let v2 = map_anon_choice_exp_0ff8d07 env v2 in
...
and map_quotable (env : env) (x : CST.quotable) =
  (match x with
...
and map_return_statement (env : env) ((v1, v2) : CST.return_statement) =
  ...
  let v2 =
    (match v2 with
    | Some x -> R.Option (Some (
        map_anon_choice_exp_3c18676 env x
      ))
    | None -> R.Option None)
  in
  R.Tuple [v1; v2]
...
and map_short_function_definition (env : env) ((v1, v2, v3) : CST.short_function_definition) =
...
and map_typed_parameter (env : env) (x : CST.typed_parameter) =
  (match x with
  | `Opt_choice_id_COLONCOLON_prim_exp_opt_where_clause (v1, v2, v3, v4) -> R.Case ("Opt_choice_id_COLONCOLON_prim_exp_opt_where_clause",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            (match x with
